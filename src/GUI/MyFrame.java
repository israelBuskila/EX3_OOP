...
public class MyFrame extends JFrame implements MouseListener
{
	...
	private void initGUI() 
	{
		...
		saveToKml.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionEvent) {
				...
				kml.writeToKML(getWidth(), getHeight(),g1, "outputKML");
	...
	public class ThreadsClass extends Thread {
		...
		private MyFrame windows = new MyFrame();
		int n=0;

		public ThreadsClass(ArrayList<Point3D> waze,MyFrame windows, int n) {
			this.waze = waze;
		...
		public void run() {
			for (int i = 0; i < waze.size(); i++) {
				...
				this.windows.getPackman().get(n).setP(p);
...
//		Game a=new Game(packman,fruit);
//		ShortestPathAlgo sh=new ShortestPathAlgo(a,getW(),getH(),true);
//		ArrayList<Path> waze=new ArrayList<Path>(sh.getPaths());
//		if(! waze.isEmpty())
//		for (int i = 0; i < waze.get(0).getPath().size()-1; i++) {
//			int x1 = (int) waze.get(0).getPath().get(i).x();
//			int y1 = (int) waze.get(0).getPath().get(i).y();
//			int x2 = (int) waze.get(0).getPath().get(i+1).x();
//			int y2 = (int) waze.get(0).getPath().get(i+1).y();
...
//		}
	...
	public void mouseClicked(MouseEvent arg) {
		...
		double yn = arg.getY();
		Map m=new Map();
		m.PixelToCoords(this.getWidth(), this.getHeight(), xn, yn);
		x = arg.getX();
		y = arg.getY();
